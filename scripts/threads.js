...
async function sendData(form) {
  try {
    ...
    await makeRequest(
      "http://localhost:1337/api/threads",
      "POST",
      JSON.stringify(data),
      {
...
function printData(data) {
  const lista = document.getElementById("accordionExample");
  let nombreSerie = data.data.attributes.name;
  let threads = data.data.attributes.threads.data;
  const name = document.createElement("h4");
  name.classList.add("name");
  name.textContent = nombreSerie;
  ...
  for (const thread of threads) {
    ...
    div.innerHTML = `
      ...
      count === 0 ? "show" : " "
          ...
          <div class="accordion-body">
          <a href="comments.html?id=${thread.id}">${
...
if (serieID) {
  await makeRequest(
    `http://localhost:1337/api/series/${serieID}?populate=threads`,
    "GET",
    null,
    {
      Authorization: `Bearer ${userToken}`,
